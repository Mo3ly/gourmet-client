                ...
                <div class="-mb-px flex px-4 space-s-8">
                    <div v-for="resturant in resturants" :key="resturant.name_en" class=" w-24">
                        <nuxt-link :to="localePath(`/resturant/${resturant.name_en}`)">
                            <img class=" h-16 rounded w-full object-cover object-center"
                                    :src="resturant.image"
                                    :alt="resturant.name_en"
                            />
    ...
    <section aria-labelledby="products-heading" class="pt-6 pb-24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-x-8 gap-y-10">
            ...
            <div v-if="resturant.categories.length" class="hidden lg:block pt-4">
                ...
                <li 
                    :class="[category.name_en === currentlyActiveToc ? 'border-e-2 pl-4 border-pink-500 text-pink-500 mb-5' : 'pl-4 mb-5', 'hover:text-pink-500 transform transition hover:scale-95']" 
                    v-for="category in resultQuery" :key="category.id">
            ...
            <div v-if="resturant.categories.length" class="pt-4 hidden lg:block md:block">
                <div class="p-3 bg-pink-200 bg-opacity-30 dark:bg-opacity-40 font-semibold  rounded-md text-gray-800 dark:text-gray-300 text-xl">
...
export default {
    ...
    computed: {
        ...
        resultQuery() {
            if (this.searchQuery) {
                ...
                return this.resturant.categories.filter(category => 
                    category.products
                        .some(product => locale === 'ar' ? 
                            product.name_ar.toLowerCase().includes(this.searchQuery) : 
                            product.name_en.toLowerCase().includes(this.searchQuery))
                );
            } else {
    ...
    mounted() {
        ...
        document
        .querySelectorAll(".menuContent div[id]")
        .forEach(section => {
    ...
    async asyncData({ params, $axios, error  }) {
        ...
        // get resturant
        const resturant = await $axios.get(`/api/resturants/${params.resturant}`)
        .then((response) => {
            return response.data
        })
        .catch((e) => {
