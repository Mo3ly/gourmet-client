          ...
          <div class="w-full p-4 px-5 py-5">
            <div class="md:grid md:grid-cols-3 gap-2 ">
              <div class="col-span-2 p-5">
                <div class="pt-4">
                  <div class="p-3 bg-pink-200 bg-opacity-30 dark:bg-opacity-80 font-semibold  rounded-md text-gray-800 text-xl"><i class="gd-icons icon-basket-simple pb-1"></i> {{ $t("cart.name") }}</div>
  ...
  export default {
    ...
    methods: {
      ...
      async checkout(cart, table) {
        ...
        const { value: tableNumber } =
          table != null && this.inRange(table, 1, 50)
            ? { value: table }
        ...
        if (tableNumber) {
          ...
          let userData = await this.$axios
            .get("https://api.bigdatacloud.net/data/ip-geolocation-full?key=" + apiKey, { baseUrl: null })
            .then((r) => {
              return r;
            })
          ...
          let userIP = null;
          if (userData != "" && userData != null && userData != undefined) userIP = userData.data.ip;
          else userIP = "empty";
