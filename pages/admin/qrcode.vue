<template>
    <div>
        <h3 class="text-gray-700 text-3xl font-bold">QR Codes</h3>  
        <div class="grid grid-cols-3 gap-4 mt-4">
            <div class="col-span-1">
                <div class="col-span-6 sm:col-span-4">
                    <label for="table_number" class="block text-sm font-medium text-gray-700">Table number</label>
                    <input type="number" max="500" v-model="tableNumber" name="table_number" id="table_number" autocomplete="off" :class="[tableNumber === '' ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : '']" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>
            ...
            <div>                 
                <span class="font-bold">Genereted URL:</span> <a :href="QRCode" target="_blank"> {{ QRCode }} </a>
            </div>
...
export default {
  data() {
      return {
        url: process.env.URL,
        resturants: ['resturants', 'Little Italy', 'Melt'],
  ...
  computed: {
    QRCode: function () {
      return this.selectedResturant === null || this.selectedResturant === 'resturants' ? 
        `${this.url}resturants?table=${this.tableNumber}` :  
        `${this.url}resturant/${this.selectedResturant}?table=${this.tableNumber}`
    }
  },
  methods: {
      async print () {
