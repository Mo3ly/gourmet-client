        ...
        <div class="me-4 flex-1 flex flex-col">
            <div>
                <div class="flex justify-between text-base font-semibolds text-gray-900 dark:text-gray-100">
                    <h3>
                        <a href="#"> <span v-if="getCart.find(item => item.id == product.id)" class="text-pink-500"> {{ getCart.find(item => item.id == product.id).quantity }}x </span> {{ $i18n.locale == 'en' ? product.name_en : product.name_ar }}  </a>
                    </h3>
