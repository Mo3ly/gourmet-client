<template>
  <div class="flex justify-between items-center mt-5 pt-3">
    <div class="flex items-center">
      <img :title="`Notes: ${item.notes != null && item.notes != undefined && item.notes != '' ? item.notes : 'nothing'}`" :src="item.image" alt="product image" class="rounded-md w-10 h-10" />
    ...
    <div class="flex justify-center items-center">
      ...
      <div class="pe-3 flex">
        <div class="flex flex-row bg-transparent">
          <button v-if="item.quantity == 1" @click="deleteCartItem({ id: item.id, notes: item.notes })" data-action="decrement" class=" bg-red-300 text-red-600 hover:text-red-700 hover:bg-red-400 h-6 w-6 rounded-full  cursor-pointer">
  ...
  export default {
    ...
    methods: {
      ...
      increaseQuantity() {
        ...
        this.quantity++;
        this.updateCartItem({ id: this.item.id, quantity: this.quantity, notes: this.item.notes });
      },
      decreaseQuantity() {
        this.quantity = this.item.quantity;
        this.quantity > 1 ? this.quantity-- : this.quantity, this.updateCartItem({ id: this.item.id, quantity: this.quantity, notes: this.item.notes });
      },
    },
  };
</script>
