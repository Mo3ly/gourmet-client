<template>
  <button
    class="relative overflow-hidden px-3 lg:px-4 flex items-center text-gray-900 bg-gray-100 hover:bg-gray-200 focus:bg-gray-300 dark:bg-gray-200 rounded h-10 outline-none ease-linear transform active:scale-95 transition-transform duration-100"
    @click="toggleDarkTheme"
  >
    <span
      class="relative overflow-hidden w-5 h-5 flex items-center justify-center"
    >
      <MoonIcon
...
export default {
  ...
  methods: {
    toggleDarkTheme() {
      ...
      //document.querySelector('meta[name="theme"]').setAttribute("content", '#000000');//this.$colorMode.preference === 'light' ? '#000000' : '#ffffff'
