<template>
  <div :class="['bottom-sheet dark:bg-dark-surface', { opened: opened, closed: opened === false, moving: moving }]" v-on="handlers" ref="bottomSheet">
    <div v-if="overlay" class="bottom-sheet__backdrop" :style="{ background: overlayColor }" />
    <div :style="[{ bottom: cardP + 'px', maxWidth: maxWidth, maxHeight: maxHeight }, { height: isFullScreen ? '100%' : 'auto' }]" :class="['bottom-sheet__card bg-white dark:bg-gray-900', { stripe: stripe, square: !rounded }, effect]" ref="bottomSheetCard">
      <div class="bottom-sheet__pan" ref="pan">
        <div class="bottom-sheet__bar" />
      </div>
      <div :style="{ height: contentH }" ref="bottomSheetCardContent" class="bottom-sheet__content">
        <slot />
      </div>
    </div>
  </div>
    ...
    methods: {
      ...
      init() {
        return new Promise((resolve) => {
          ...
          this.cardP = this.effect === "fx-slide-from-right" || this.effect === "fx-slide-from-left" ? 0 : `-${this.cardH + this.stripe}px`;
          if (!this.inited) {
            this.inited = true;
            // let options = {
            //   recognizers: [
