<template>
    <div class="p-2 bg-gray-200 rounded mb-3">
        <div class="text-black mb-2 font-bold">Add new product</div>
            ...
            <div class="col-span-6 sm:col-span-3 ">
                ...
                <div class="items-center flex space-s-4">
                    <div class="flex items-center">
                        <input id="isNew" v-model="isNew" name="isNew" type="checkbox" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
                        <label for="isNew" class="ml-3 block text-sm font-medium text-gray-700">
                            New
                        </label>
                    </div>
                    <div class="flex items-center">
                        <input id="isSpecial" v-model="isSpecial" name="isSpecial" type="checkbox" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
        ...
        <div class="px-1 py-3 sm:flex sm:flex-row-reverse">
            <button @click="addNewProduct" ref="addProductButton" type="button" class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">
                Save
            </button>
            <button @click="hide" type="button" class="mt-3 inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                Cancel
            </button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        async addNewProduct(){
            this.$refs.addProductButton.disabled = true;
            ...
            this.$axios.post('/api/products/store',formData, config)
            .then(r => {
                if(r.data.success){
                    this.progress = 0;
                ...
                this.$refs.addProductButton.disabled = false;
            })
            .catch(e => {
                this.progress = 0;
