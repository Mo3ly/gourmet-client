...
<div>
  <div class="relative w-full h-52 bg-cover bg-center group rounded-lg overflow-hidden shadow-lg transition duration-300 ease-in-out" style="background-image: url('https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/f868ecef-4b4a-4ddf-8239-83b2568b3a6b/de7hhu3-3eae646a-9b2e-4e42-84a4-532bff43f397.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..R0h-BS0osJSrsb1iws4-KE43bUXHMFvu5PvNfoaoi8o');">
      <div class="absolute inset-0 bg-blue-100 bg-opacity-75 transition duration-300 ease-in-out"></div>
      <div class="relative w-full h-full px-4 sm:px-6 lg:px-4 flex items-center">
          <button @click="show = !show" class="block uppercase mx-auto shadow bg-white text-blue-600 focus:shadow-outline focus:outline-none text-xs py-1 px-2 rounded font-bold">
              New Resturant
          </button>
      </div>
  </div>   
  ...
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
      ...
      <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
        <button @click="addNewResturant" ref="addResturantButton" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">
          Add
        </button>
...
export default {
  ...
  methods: {
      async addNewResturant(){
        this.$refs.addResturantButton.disabled = true;
        ...
        this.$axios.post('/api/resturants/store',formData, config)
        .then(r => {
          if(r.data.success){
            this.progress = 0;
          ...
          this.$refs.addResturantButton.disabled = false;
        })
        .catch(e => {
          this.progress = 0;
